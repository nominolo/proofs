...
Lemma silly_ex : 
  ...
  -> odd four = yes.
Proof.
  intros eq1 eq2.  apply eq1.  exact eq2.
Qed.
...
Proof.
  intros X l l' H.
  rewrite H. rewrite reverse_reverse. reflexivity.
Qed.
...
Lemma sillyex2 : forall (X : Set) (x y z : X) (l j : list X),
  ...
  -> x = z.
Proof.
  intros X x y z l j. intros contra eq. inversion contra.
Qed.
...
(* Exercise: The above lemma can also be proved by induction on
   [n] (though we have to be a little careful about which order we
...
Proof.
  ...
  Case "O".
    intro m. destruct m. 
    Case "O". simpl. intro; trivial.
    Case "S". simpl. intro contra. inversion contra.
  Case "S".
    intro m. destruct m.
    Case "O". simpl. intro contra. inversion contra.
    Case "S". simpl. intro H. apply IHn in H. rewrite H. trivial.
Qed.
